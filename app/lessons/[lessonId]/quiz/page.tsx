...
function getQuestions(lessonId: number): Q[] {
  if (lessonId === 1) {
    return [
  ...
  if (lessonId === 2) {
    return [
      ...
      { q: "Is A a vowel?", options: ["Yes", "No", "Maybe"], answer: 0 },
  ...
  if (lessonId === 4) {
    return [
      ...
      { q: "A dining table is in the...", options: ["Dining room", "Roof", "Car"], answer: 0 },
    ];
  }
...
export default function QuizPage() {
  ...
  const questions: Q[] = useMemo(() => getQuestions(lessonId), [lessonId]);

  const [answers, setAnswers] = useState<(number | null)[]>([]);
  const [saving, setSaving] = useState(false);

  // Reset answers when lesson changes or questions load
  useEffect(() => {
    setAnswers(Array(questions.length).fill(null));
  }, [questions.length]);

  // If no quiz exists for that lesson, show a friendly message
  if (!lessonId || Number.isNaN(lessonId)) {
    return (
      <div style={{ padding: 24 }}>
        <h1>Quiz not found</h1>
        <p>Invalid lesson id.</p>
        <button onClick={() => router.push("/lessons")} style={{ padding: "10px 14px", borderRadius: 10 }}>
          Back to Lessons
        </button>
      </div>
    );
  ...
  const score = answers.reduce<number>((acc, a, i) => {
    ...
    return a === questions[i].answer ? acc + 1 : acc;
  }, 0);

  const passScore = 7; // 7/10 to pass
  ...
  return (
    <div style={{ padding: 24, maxWidth: 900, margin: "0 auto" }}>
      ...
      <p style={{ opacity: 0.8, marginBottom: 24 }}>
        Score: {score}/{questions.length} (Pass: {passScore}/{questions.length})
      </p>
